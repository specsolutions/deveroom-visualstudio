...
namespace Deveroom.VisualStudio.Specs.StepDefinitions
{
    ...
    public class DeveroomSteps : Steps
    {
        ...
        private readonly List<Action<StubProjectScope>> _projectScopeConfigurationSteps = new List<Action<StubProjectScope>>();
        ...
        public void GivenThereIsASimpleSpecFlowProjectWithExternalBindingsForVersion(NuGetVersion specFlowVersion)
        {
            _generatorOptions = new GeneratorOptions
            {
                ...
                ExternalBindingPackageName = $"Deveroom.SampleSpecFlow{specFlowVersion.ToShortVersionString()}.ExternalBindings"
            };
        }
        ...
        public void GivenThereIsASimpleSpecFlowProjectWithPlatformTargetForVersion(string platformTarget, NuGetVersion specFlowVersion)
        {
        ...
        public void ThenTheDiscoverySucceedsWithSeveralStepDefinitions()
        {
            ...
            _bindingRegistry.StepDefinitions.Should().HaveCountGreaterThan(1, "there should be step definitions discovered");
        }
        ...
        {
            _bindingRegistry.Should().NotBeNull();
            var unicodeBinding = _bindingRegistry.StepDefinitions.FirstOrDefault(sd => sd.Regex.ToString().Contains("Unicode"));
        ...
        public void ThenTheStepDefinitionsContainSourceFileAndLine()
        {
            ...
            {
                stepDefinitionBinding.Implementation.SourceLocation?.SourceFile.Should().NotBeNull($"The step defintion '{stepDefinitionBinding.Implementation.Method}' should contain source file");
                File.Exists(stepDefinitionBinding.Implementation.SourceLocation?.SourceFile).Should().BeTrue($"The step defintion source '{stepDefinitionBinding.Implementation.SourceLocation?.SourceFile}' should point to a valid file");
                stepDefinitionBinding.Implementation.SourceLocation?.SourceFileLine.Should().BeGreaterThan(1, $"The step defintion '{stepDefinitionBinding.Implementation.Method}' should contain source file line");
            }
        }
        ...
        private string GetAFeatureFile()
        {
            return ProjectGenerator.FeatureFiles.First();
        }
